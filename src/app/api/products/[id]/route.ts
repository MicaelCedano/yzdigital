    ...
    const updated = await prisma.$transaction(async (tx) => {
      const prod = await tx.product.update({
        ...
        data: {
          ...
          imageUrl: imageUrl !== undefined ? toImageUrl(imageUrl) || null : existing.imageUrl,
      ...
      if (price !== undefined) {
        ...
        if (defaultList) {
          await tx.productPrice.upsert({
            ...
            update: {
              priceTier1: priceNum,
              priceTier2: priceTier2 == null ? Math.max(0, priceNum - 100) : Number(priceTier2) || 0,
              priceTier3: priceTier3 == null ? Math.max(0, priceNum - 200) : Number(priceTier3) || 0,
              isActive: true,

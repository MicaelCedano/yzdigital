...
function LoginFormContent() {
  ...
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
    ...
    // Animación interactiva de celulares de YZ Digital
  useEffect(() => {
    ...
    // Plataformas flotantes para la escena de combate
    ...
    const HIT_TEXTS = ['POW!', 'BAM!', 'IMPACTO!', 'K.O.!', 'METEOR!', 'CRASH!', 'FLY!'];
    ...
    class PhoneFighter {
      ...
      findTarget(allFighters: PhoneFighter[]) {
        let closest = null;
      ...
      takeDamage(dmg: number, knockAngle: number, baseKnock: number) {
        ...
        if (this.damagePercent > 90) {
          spawnImpact(this.x, this.y, '#ff0055', 'IMPACTO!');
        }
      }
      ...
      update(allFighters: PhoneFighter[]) {
        this.animFrame++;
            ...
            setTimeout(() => {
              ...
              if (curDist < 75) {
                ...
                if (isMeteor) {
                  ...
                  angle = isPowerHit ? (this.facing > 0 ? -0.35 : -2.8) : this.facing > 0 ? -0.7 : -2.4;
                }
                let baseKnock = isPowerHit ? 8 : isMeteor ? 7 : 5;
                let dmg = isPowerHit ? 16 : 10;
                ...
                spawnImpact(
                  ...
                  this.brand.screen,
                  isMeteor ? 'METEOR!!' : isPowerHit ? 'POWER!!' : 'POW!'
                );
              }
            }, 120);
      ...
      drawDamagePercent(ctx: CanvasRenderingContext2D) {
    ...
    const fighters: PhoneFighter[] = [];
    for (let i = 0; i < 6; i++) {
      fighters.push(new PhoneFighter(i));
    }
    ...
    function drawPhoneStage(ctx: CanvasRenderingContext2D) {

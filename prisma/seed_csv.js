...
// Modelos adicionales de la nueva captura (XIAOMI, APPLE, JBL, ACCESORIOS, etc.)
const additionalModels = [
  // XIAOMI
  { brand: 'XIAOMI', model: 'REDMI A7 PRO', capacity: '6+64GB', price: 5749, inActiveList: true },
  { brand: 'XIAOMI', model: 'REDMI A7 PRO', capacity: '8+128GB', price: 6625, inActiveList: true },
  { brand: 'XIAOMI', model: 'REDMI 15C', capacity: '8+128GB', price: 7249, inActiveList: true },
  { brand: 'XIAOMI', model: 'REDMI 15C', capacity: '8+256GB', price: 8549, inActiveList: true },
  { brand: 'XIAOMI', model: 'REDMI 17 4+4', capacity: '8+128GB', price: 9849, inActiveList: true },

  // APPLE
  { brand: 'APPLE', model: 'IPHONE 17 PRO MAX SELLADO', capacity: '256GB', price: 0, inActiveList: true },

  // BOCINA JBL
  { brand: 'BOCINA JBL', model: 'JBL CLIP 5', capacity: 'Portátil', price: 2895, inActiveList: true },
  { brand: 'BOCINA JBL', model: 'JBL FLIP 5', capacity: 'Bluetooth', price: 5300, inActiveList: true },

  // ACCESORIOS
  { brand: 'ACCESORIOS', model: 'AIRPODS 4 HOTWAV SEVEN', capacity: 'Bluetooth 5.3', price: 390, inActiveList: true },
  { brand: 'ACCESORIOS', model: 'AIRPODS ZTE BUDS A9', capacity: 'In-Ear', price: 250, inActiveList: true },
  { brand: 'ACCESORIOS', model: 'BOCINA WHOOP VIDE WIRELESS SPEAKER', capacity: 'Wireless', price: 650, inActiveList: true },
  { brand: 'ACCESORIOS', model: 'CARGADOR OTUX ONE USB/TIPO C', capacity: '12W', price: 99, inActiveList: true },
  { brand: 'ACCESORIOS', model: 'CARGADOR OTUX ONE POWER 1 TIPO C', capacity: '20W', price: 160, inActiveList: true },
];

async function main() {
  console.log('--- Cargando datos y colores modernos ---');
  ...
  // Colores vivos y modernos por marca/categoría exactamente como en la imagen
  const brandHeaders = [
    { brand: 'SAMSUNG', color: '#00A3E0', order: 1 },
    { brand: 'XIAOMI', color: '#F97316', order: 2 },
    { brand: 'MOTOROLA', color: '#C026D3', order: 3 },
    { brand: 'CUBOT', color: '#2E7D32', order: 4 },
    { brand: 'APPLE', color: '#18181B', order: 5 },
    { brand: 'TECNO', color: '#F59E0B', order: 6 },
    { brand: 'OUKITEL', color: '#0F766E', order: 7 },
    { brand: 'VORTEX', color: '#BE123C', order: 8 },
    { brand: 'BLU', color: '#1E293B', order: 9 },
    { brand: 'ZTE', color: '#2563EB', order: 10 },
    { brand: 'M-HORSE', color: '#4F46E5', order: 11 },
    { brand: 'COOLPAD', color: '#7C3AED', order: 12 },
    { brand: 'ITEL', color: '#E11D48', order: 13 },
    { brand: 'TCL', color: '#0284C7', order: 14 },
    { brand: 'TABLETAS', color: '#0284C7', order: 15 },
    { brand: 'BICICLETAS', color: '#0F172A', order: 16 },
    { brand: 'BOCINA JBL', color: '#EA580C', order: 17 },
    { brand: 'ACCESORIOS', color: '#0284C7', order: 18 },
    { brand: 'TELEVISION', color: '#0369A1', order: 19 },
    { brand: 'SUNELAN', color: '#059669', order: 20 },
    { brand: 'ALCATEL', color: '#475569', order: 21 },
    { brand: 'KARGAMAX', color: '#D97706', order: 22 },
  ];
  ...
  for (const bh of brandHeaders) {
    const cat = await prisma.category.create({
      data: {
        name: bh.brand,
        slug: bh.brand.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
        headerColor: bh.color,
        sortOrder: bh.order,
      },
    });
    categoryMap.set(bh.brand.toUpperCase(), cat);
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const idRaw = cols[0];
    const modelo = cols[2];
    const marcaRaw = (cols[4] || 'OTROS').toUpperCase();
  ...
  // 2. Insertar Adicionales
  for (const item of additionalModels) {
    let cat = categoryMap.get(item.brand.toUpperCase());
    if (!cat) {
      cat = await prisma.category.create({
        data: {
      ...
      categoryMap.set(item.brand.toUpperCase(), cat);
    }

    let sku = `${item.brand}-${item.model}-${item.capacity}`.replace(/\s+/g, '-');
    if (createdSkus.has(sku)) {
      sku = `${sku}-${count}`;
    }
    createdSkus.add(sku);
    ...
    const product = await prisma.product.create({
      data: {
        ...
        stock: 20,
        sortOrder: count,
        inActiveList: item.inActiveList,
        isActive: true,
        categoryId: cat.id,
      },
    });
    ...
    await prisma.productPrice.create({
      data: {
        ...
        priceListId: listaGeneral.id,
        currency: 'DOP',
        priceTier1: item.price,
        priceTier2: item.price,
        priceTier3: item.price,
        isActive: true,
        createdById: adminUser.id,
      },
    });
  ...
  console.log(`--- Se crearon ${count} productos exitosamente con estilos modernos ---`);
}
